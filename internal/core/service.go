...
type TopUpRequest struct { UserID string; Amount int }
...
func (s *Store) TopUp(ctx context.Context, req TopUpRequest) error {
	if req.Amount <= 0 { return fmt.Errorf("invalid amount") }
...
func (s *Store) EnqueueAndCharge(ctx context.Context, r SendRequest) (msgID string, already bool, err error) {
	...
	if r.IdempotencyKey != nil {
		...
		if err == nil { return existsID, true, tx.Commit(ctx) }
		if err != pgx.ErrNoRows { return "", false, err }
	}
	...
	if err != nil { return "", false, err }
	if balance < PricePerSMS { return "", false, errInsufficientBalance }

	// Debit and enqueue message.
	_, err = tx.Exec(ctx, `UPDATE users SET balance = balance - $1 WHERE id=$2`, PricePerSMS, r.UserID)
	if err != nil { return "", false, err }

	if r.IdempotencyKey == nil { empty := ""; r.IdempotencyKey = &empty }
	err = tx.QueryRow(ctx, `
...
func (s *Store) ClaimQueuedMessages(ctx context.Context, limit int) ([]string, error) {
	...
	if len(ids) == 0 { return nil, tx.Commit(ctx) }
...
func (s *Store) QueryMessages(ctx context.Context, userID string, status *string, from, to *time.Time, limit, offset int) ([]Message, error) {
	...
	args := []any{userID}
	idx := 2
	if status != nil { q += fmt.Sprintf(" AND status=$%d", idx); args = append(args, *status); idx++ }
	if from != nil { q += fmt.Sprintf(" AND requested_at >= $%d", idx); args = append(args, *from); idx++ }
	if to != nil { q += fmt.Sprintf(" AND requested_at < $%d", idx); args = append(args, *to); idx++ }
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.UserID, &m.To, &m.Body, &m.Status, &provID, &errCode, &m.RequestedAt, &sentAt, &delAt, &m.Attempts); err != nil { return nil, err }
		m.ProviderMessageID = provID; m.ErrorCode = errCode; m.SentAt = sentAt; m.DeliveredAt = delAt
		out = append(out, m)
	}
	return out, nil
}